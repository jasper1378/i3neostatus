...
private:
  shared_state()
      : m_state{STATE::READ}, m_value{nullptr}, m_exception{nullptr} {}
...
public:
  bool set_value(std::unique_ptr<t_value> value) {
    if (m_state.load() != STATE::EXCEPTION) {
      t_value *old_value{m_value.exchange(value.release())};
      m_state.store(STATE::UNREAD);
      m_state.notify_all();
      if (old_value) {
        delete old_value;
        old_value = nullptr;
